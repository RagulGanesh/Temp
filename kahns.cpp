...
int main()
{
    ...
    {
        cin >> x >> y;
        indegree[y - 1]++;               // Incrementing indegree
        AdjList[x - 1].push_back(y - 1); // Add the edge x -> y to adjacency list
    }

    for (int i = 0; i < v; i++)
        if (indegree[i] == 0)
            q.push(i); // Step 2: Add all indegree 0 nodes to queue
    ...
    while (!q.empty()) // Step 3: Remove vertex from queue
    {
        ...
        for (auto x : AdjList[q.front()])
        {
            ...
            if (indegree[x] == 0)
                q.push(x); // Step 3.3 Push adjacent node with indegree 0
        }
